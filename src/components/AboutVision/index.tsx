{/* <section className="bg-gray-800 text-white py-12 px-4">
        <h2 className="text-3xl font-bold text-center mb-4">Our Vision</h2>
        <p className="text-xl text-center">
          We envision a future where everyone has access to the education and skills they need to thrive in the ever-evolving job market. We see CareerConnect as a catalyst for empowering individuals to unlock their full potential and achieve their career aspirations. By providing a comprehensive and accessible platform for learning, we aim to bridge the skills gap and create a world where education is a lifelong journey, not just a stepping stone.
        </p>
      </section> */}

export const AboutVision = () => {
  return (
    <section className="container text-center py-16">
      <div className="bg-muted/50 border rounded-lg py-12 text-centerrt p-8 flex flex-col gap-8">
        <h2 className="text-3xl md:text-4xl font-bold">Our{" "}
          <span className="bg-gradient-to-b from-primary/60 to-primary text-transparent bg-clip-text">Vision</span>
        </h2>

        <p className="text-xl text-muted-foreground">
          We envision a future where everyone has access to the skills and resources they need to thrive in the ever-evolving job market. We see CareerConnect as a catalyst for empowering individuals to unlock their full potential and achieve their career aspirations. By providing a comprehensive and accessible platform for learning, we aim to bridge the skills gap and create a world where education is a lifelong journey, not just a stepping stone.
        </p>
      </div>
    </section>
  )
};
